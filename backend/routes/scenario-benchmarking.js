// Auto-generated feature route: scenario-benchmarking
// Domain: scenario library curator — Capability: benchmarking scenarios and diffing results
// Mounted under: /api/scenario-benchmarking
...
// POST /api/scenario-benchmarking/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a scenario library curator. Your task is benchmarking scenarios and diffing results. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/scenario-benchmarking/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/scenario-benchmarking/:id — fetch one
...
// DELETE /api/scenario-benchmarking/:id — discard a run
