...
// Route: /api/gap-ai-scenario-coverage-analyzer
// Feature: Ai Scenario Coverage Analyzer
// Description: (what scenarios are under-explored)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Scenario Coverage Analyzer. (what scenarios are under-explored)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Scenario Coverage Analyzer\nDescription: (what scenarios are under-explored)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-scenario-coverage-analyzer', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-scenario-coverage-analyzer', title: 'Ai Scenario Coverage Analyzer', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-scenario-coverage-analyzer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-scenario-coverage-analyzer']);
      return res.json({ slug: 'ai-scenario-coverage-analyzer', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-scenario-coverage-analyzer', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-scenario-coverage-analyzer', items: [], error: (e && e.message) || 'Server error' });
  }
});
