// Auto-generated feature route: negotiation-agent
// Domain: multi-agent negotiation agent — Capability: running consensus or deal-making scenarios
// Mounted under: /api/negotiation-agent
...
// POST /api/negotiation-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a multi-agent negotiation agent. Your task is running consensus or deal-making scenarios. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/negotiation-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/negotiation-agent/:id — fetch one
...
// DELETE /api/negotiation-agent/:id — discard a run
