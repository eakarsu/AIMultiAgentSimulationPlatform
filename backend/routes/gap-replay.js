...
// Route: /api/gap-replay
// Feature: Replay
// Description: Replay UI (route exists but limited frontend)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Replay. Replay UI (route exists but limited frontend)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'replay', title: 'Replay', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'replay', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['replay']);
      return res.json({ slug: 'replay', items: r.rows || [] });
    }
    return res.json({ slug: 'replay', items: [] });
  } catch (e) {
    return res.json({ slug: 'replay', items: [], error: (e && e.message) || 'Server error' });
  }
});
