// Auto-generated feature route: behavior-steering-stream
// Domain: real-time agent steering — Capability: adjusting parameters mid-simulation
// Mounted under: /api/behavior-steering-stream
...
// POST /api/behavior-steering-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a real-time agent steering. Your task is adjusting parameters mid-simulation. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/behavior-steering-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/behavior-steering-stream/:id — fetch one
...
// DELETE /api/behavior-steering-stream/:id — discard a run
