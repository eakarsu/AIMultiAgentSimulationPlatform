...
// Route: /api/gap-websocket
// Feature: Websocket
// Description: WebSocket streaming for live runs
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Websocket. WebSocket streaming for live runs' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'websocket', title: 'Websocket', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'websocket', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['websocket']);
      return res.json({ slug: 'websocket', items: r.rows || [] });
    }
    return res.json({ slug: 'websocket', items: [] });
  } catch (e) {
    return res.json({ slug: 'websocket', items: [], error: (e && e.message) || 'Server error' });
  }
});
