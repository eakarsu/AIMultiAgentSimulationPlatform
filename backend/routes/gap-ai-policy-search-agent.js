...
// Route: /api/gap-ai-policy-search-agent
// Feature: Ai Policy Search Agent
// Description: (RL-style policy discovery)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Policy Search Agent. (RL-style policy discovery)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Policy Search Agent\nDescription: (RL-style policy discovery)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-policy-search-agent', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-policy-search-agent', title: 'Ai Policy Search Agent', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-policy-search-agent', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-policy-search-agent']);
      return res.json({ slug: 'ai-policy-search-agent', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-policy-search-agent', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-policy-search-agent', items: [], error: (e && e.message) || 'Server error' });
  }
});
