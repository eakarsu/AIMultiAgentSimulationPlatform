// Auto-generated feature route: vertical-pack
// Domain: vertical scenario librarian — Capability: configuring supply chain, market, disease spread, traffic packs
// Mounted under: /api/vertical-pack
...
// POST /api/vertical-pack/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a vertical scenario librarian. Your task is configuring supply chain, market, disease spread, traffic packs. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vertical-pack/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vertical-pack/:id — fetch one
...
// DELETE /api/vertical-pack/:id — discard a run
